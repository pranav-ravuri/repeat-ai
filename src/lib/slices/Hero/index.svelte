<script lang="ts">
	...
	onMount(() => {
		...
		// tl.fromTo('.hero__glow--one', { y: 20 }, { y: 0, opacity: 1, duration: 1.2 }, "-=1");

		// tl.fromTo('.hero__glow--two', { y: 20 }, { y: 0, opacity: 1, duration: 1.2 }, "-=1");
	});
</script>
...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	<div class="relative text-center">
		...
		{#if slice.primary.body}
			<div class="hero__body mx-auto mt-6 max-w-md text-balance text-gray-300 opacity-0">
		...
		{#if slice.primary.image}
			<div class="hero__image glass-container mt-16 w-fit opacity-0">
				<div
					class="hero__glow--one hero__glow absolute top-0 left-1/3 -z-10 h-2/3 w-2/3 bg-violet-700/50 opacity-0 mix-blend-screen blur-3xl filter md:blur-[120px]"
				></div>
				<div
					class="hero__glow--two hero__glow absolute top-1/3 left-0 -z-10 h-2/3 w-2/3 bg-orange-600/50 opacity-0 mix-blend-screen blur-3xl filter md:blur-[120px]"
				></div>
